#!/usr/bin/env node
/**
 * Turn 4 — test drive intent: "Can I book a test drive for that one?"
 * Requires prior SUV recommendations + vehicle reference in the same test conversation.
 *
 * Usage:
 *   npx @railway/cli run node scripts/controlled-whatsapp-test-drive-test.js
 */
...
const TEST_MESSAGE = "Can I book a test drive for that one?";
...
async function main() {
    ...
    const wamid = `wamid.test-drive-${Date.now()}`;
    const payload = {
        object: "whatsapp_business_account",
        entry: [
            {
                id: "WABA_TEST_DRIVE_TEST",
                changes: [
                    {
                        field: "messages",
                        value: {
    ...
    console.log("Test drive turn 4 test");
    ...
    console.log("\nWaiting 55s for worker to process...");
    await sleep(55000);

    console.log("\nInspect Railway logs for wamid prefix:", wamid.slice(0, 36));
    console.log("\nTurn 4 expectations:");
    console.log("  isNewConversation: false");
    console.log("  Vehicle reference resolved (CM-WP-23386 or preferred vehicle)");
    console.log("  checkTestDriveAvailability pre-loaded or called");
    console.log("  Lead stage advances toward TEST_DRIVE_REQUESTED");
