 ...
 */
export function extractSchedulingFromText(text) {
    const raw = String(text || "").trim();
    if (!raw || DAY_REF.test(raw)) return {};

    let parsed = parseScheduledInput({ scheduledAt: raw });
    if (!parsed.ok) {
        parsed = parseScheduledInput({ date: raw });
    }
    if (!parsed.ok) return {};
...
export function enrichToolArgsWithScheduling(toolName, args = {}, scheduling = {}) {
    ...
    const enriched = { ...args };
    const resolvedDate = enriched.date || enriched.scheduledAt || scheduling.pendingDate || scheduling.lastMentionedDate;

    if (!enriched.date && !enriched.scheduledAt && resolvedDate) {
...
export function schedulingUpdatesFromToolResult(toolName, args = {}, result = {}) {
    ...
    if (result.code === "NEED_TIME" || result.needsTime) {
        updates.pendingTime = true;
    } else if (result.available || result.code === "AVAILABLE") {
        updates.pendingTime = null;
    }
...
export function formatSchedulingContextForPrompt(scheduling = {}) {
    if (!scheduling.pendingDate && !scheduling.lastMentionedDate) return "";

    const parts = ["SCHEDULING CONTEXT (from this conversation — use when calling tools):"];
    const date = scheduling.pendingDate || scheduling.lastMentionedDate;
    if (date) {
        parts.push(`- Date established: ${date} (pass as date to checkTestDriveAvailability).`);
    }
